...
export class HomeComponent extends ClearObservable implements OnInit {
  ...
  getNewGames(page: number) {
    ...
    this.gamesService.getLastReleasedGames(page, this.dates).pipe(takeUntil(this.destroy$), finalize(() => {
  ...
  lastWeekGames() {
    ...
    this.gamesService.getLastReleasedGames(1, this.dates).pipe(takeUntil(this.destroy$), finalize(() => {
      this.isLoading = false;
    })).subscribe(games => {
      this.total = games.count;
      this.games = games.results;
